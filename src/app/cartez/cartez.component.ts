...
export class CartezComponent {
  ...
  addPointPx(ppos: Pos, letter: string) {
    // round for simplicity
    const cords = this.pixelPosToCord(ppos);
    this.points.push({
      letter: letter,
      cords: { x: Math.round(cords.x * 100) / 100, y: Math.round(cords.y * 100) / 100 }
    })
  ...
  draw() {
    ...
    for (let i = start; i < start + total_teeth; i++) {
      let x = Math.round(i * spaces * pos_multipler) / pos_multipler; //causes many binary errors
      // start +=
